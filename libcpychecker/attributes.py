...
def register_our_attributes():
    ...
    # Handler for __attribute__((cpychecker_ignore_refcount))
    # and #ifdef WITH_CPYCHECKER_IGNORE_REFCOUNT_ATTRIBUTE
    def attribute_callback_for_ignore_refcount(*args):
        if 0:
            print('attribute_callback_for_ignore_refcount(%r)' % args)
        check_isinstance(args[0], gcc.FunctionDecl)
        fnname = args[0].name
        fnnames_ignore_refcount.add(fnname)

    gcc.register_attribute('cpychecker_ignore_refcount',
                           0, 0,
                           False, False, False,
